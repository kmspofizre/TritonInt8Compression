...
def _quant_compress_kernel_to_int4(
    ...
    input_start = x_ptr + pid_n * cols + pid_m * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    output_start = output_ptr + (pid_n * cols // COMPRESS_FACTOR + pid_m * BLOCK_SIZE) + tl.arange(0, BLOCK_SIZE)
    ...
    for i in range(COMPRESS_FACTOR):
        ...
        x_uint8 = x_scaled.cast(tl.uint8)
        out += (x_uint8 << ((COMPRESS_FACTOR - 1 - i) * 4)).cast(OUTPUT_DTYPE)
...
def quant_compress_to_int4(weights: torch.Tensor, compress_factor: int = 2, quant_block_size: int = 128) -> torch.Tensor:
    rows, cols = weights.shape
    assert weights.is_contiguous(), "Weights matrix have to be contiguous"
    assert cols % quant_block_size == 0, f"Number of cols {cols} have to be divisible by {quant_block_size=}"

        
    if compress_factor == 2:
        assert cols % 2 == 0, f"Number of cols ({cols}) have to be divisible by 2, to pack into int8"
        output_dtype = torch.uint8
        output_tl_dtype = tl.uint8
    elif compress_factor == 8:
        assert cols % 8 == 0, f"Number of cols ({cols}) have to be divisible by 8, to pack into int32"
        output_dtype = torch.uint32
        output_tl_dtype = tl.uint32
    else:
        raise ValueError(f"Invalid compress factor: {compress_factor} doesn't supported, use [2, 8] to compress into [uint8, uint32]")
    ...
    grid = lambda meta: (
        ...
        rows, n_blocks_per_row // compress_factor
    )
...
def _dequant_decompress_kernel_from_int4_to_float16(
    ...
    input_start = x_ptr + (pid_n * cols_compressed + pid_m * BLOCK_SIZE) + tl.arange(0, BLOCK_SIZE)
    output_start = output_ptr + (pid_n * cols_original + pid_m * BLOCK_SIZE * COMPRESS_FACTOR) + tl.arange(0, BLOCK_SIZE)
    scale_start = scale_ptr + (pid_n * cols_scale + pid_m * COMPRESS_FACTOR) 
    x = tl.load(input_start)

    for _ in range(COMPRESS_FACTOR):
        x_local = x & 0xF

        scale = tl.load(scale_start)

        out = (x_local - 8).to(tl.float16) * scale
        tl.store(output_start, out)
...
def dequant_decompress_from_int4_to_float16(weights_compressed: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    ...
    assert rows == rows_scale, f"Compress weights and scale must have the same number of rows"
    ...
    output = torch.empty((rows, cols_original), device=weights_compressed.device, dtype=torch.float16)

    print(f""" DEQUANT LOGS 
          {cols_original=}
          {cols_compressed=}
          {cols_scale=}
          {compress_factor=}
    ...
    grid = lambda meta: (
        # triton.cdiv(rows, meta["BLOCK_M"]),
        # triton.cdiv(IN, meta["BLOCK_N"]),
        rows, cols_original // (quant_block_size * compress_factor)
    )
