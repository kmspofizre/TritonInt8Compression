...
def _quant_compress_kernel_to_int4(
    ...
    input_start = x_ptr + (pid_n * cols + pid_m * BLOCK_SIZE * COMPRESS_FACTOR) + tl.arange(0, BLOCK_SIZE)
...
def dequant_decompress_from_int4_to_float16(weights_compressed: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    ...
    assert rows == rows_scale, "Compress weights and scale must have the same number of rows"
